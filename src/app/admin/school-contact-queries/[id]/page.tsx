...
import { getSchoolContactQueryDetail } from "@/server/queries/school-contact-queries";
...
export default async function AdminSchoolContactQueryDetailPage({
  params,
}: {
  ...
  const query = await getSchoolContactQueryDetail(id);
  if (!query) notFound();
  ...
  return (
    <div className="max-w-2xl flex flex-col gap-lg">
      <BackButton />
      <div>
        <h1 className="font-headline-lg text-headline-lg text-on-surface">{query.institutionName}</h1>
        <p className="font-body-sm text-body-sm text-on-surface-variant mt-1">{query.queryUid}</p>
      </div>

      <div className="bg-surface-container-lowest border border-surface-variant rounded-xl p-lg">
        <Row label="Contact person" value={query.contactPersonName} />
